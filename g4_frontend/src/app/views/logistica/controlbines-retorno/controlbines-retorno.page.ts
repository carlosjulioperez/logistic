...
import { GridApi, ColDef, RowEditingStoppedEvent, CellEditingStartedEvent, CellEditingStoppedEvent } from 'ag-grid-community';
...
@Component({
  selector: 'app-controlbines-retorno',
  templateUrl: './controlbines-retorno.page.html',
  styleUrls: ['./controlbines-retorno.page.scss'],
})
export class ControlbinesRetornoPage implements OnInit {
  ...
  columnDefs: ColDef[] = [   
    ...
    */
    ...
    { headerName: 'Conductor', field: 'conductor' ,  width: 180  , resizable: true ,  floatingFilter: true ,hide: true, minWidth: 180 , cellStyle: {fontSize: '9px'}  , //pinned: 'left' ,
    editable: true ,
    ...
    { headerName: 'Piloto', field: 'piloto' ,  width: 180  , resizable: true ,  floatingFilter: true ,hide: true, minWidth: 180 , cellStyle: {fontSize: '9px'}  , //pinned: 'left' ,
    editable: true ,
    /*cellRenderer: 'textbuttonRenderer' ,
    cellRendererParams: {
    ...
    { headerName: 'Copiloto', field: 'copiloto' ,  width: 180  , resizable: true ,  floatingFilter: true ,hide: true, minWidth: 180 , cellStyle: {fontSize: '9px'}  , //pinned: 'left' ,
    editable: true ,
    },  
    { headerName: 'Bines', field: 'binesretorno',  width: 100  , resizable: true , editable: true, 
        floatingFilter: true ,minWidth: 80 , cellStyle: {fontSize: '11px' , 'text-align': 'right', 'background-color': '#b1ccfb'} }, 
    { headerName: 'Calada', field: 'caladaretorno',  width: 100  , resizable: true , editable: true, 
        floatingFilter: true ,minWidth: 80 , cellStyle: {fontSize: '11px' , 'text-align': 'right', 'background-color': '#b1ccfb'} }, 
    { headerName: 'Conica', field: 'conicaretorno',  width: 100  , resizable: true , editable: true, 
        floatingFilter: true ,minWidth: 80 , cellStyle: {fontSize: '11px' , 'text-align': 'right', 'background-color': '#b1ccfb'} }, 
    { headerName: 'Meta', field: 'metaretorno',  width: 100  , resizable: true , editable: true, 
        floatingFilter: true ,minWidth: 80 , cellStyle: {fontSize: '11px' , 'text-align': 'right', 'background-color': '#b1ccfb'} }, 
    { headerName: 'Sal', field: 'salretorno',  width: 100  , resizable: true , editable: true, 
        floatingFilter: true ,minWidth: 80 , cellStyle: {fontSize: '11px' , 'text-align': 'right', 'background-color': '#b1ccfb'} }, 
  ...
  async buscarProductor(){  
    try {
      ...
      await this.mostrarFormularioBusqueda('Lista de Productor', this.listaproductordetalle , ["proveedor","sector","cantidadprogramada"]).then(async data => {
        if(!!data){
          console.log('(<any>data).id' , (<any>data).id)
          this.idprogramaciondetalle  = (<any>data).id
          this.productorlabel    = (<any>data).proveedor + '-' + '['+(<any>data).sector+']' + '-' + '['+(<any>data).cantidadprogramada + ']';
  ...
  async excelExportar(){
    ...
    if( this.listDetallePrograma.length == 0) {
     ...
     const wb: XLSX.WorkBook = XLSX.utils.book_new();
     XLSX.utils.book_append_sheet(wb, ws, 'ListaControBinesRetorno');
     XLSX.writeFile(wb, 'ListaControBinesRetorno' + '.xlsx');
